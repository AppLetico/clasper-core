...
.PHONY: install dev build test lint-typecheck ci setup workspace clean conformance dispatcher build-ops seed-ops dev-seeded seed-ops-seeded reset-db reset-seeded-db reset-reseed-ops-seeded seed-openclaw-policies demo-openclaw openclaw-plugin-reload dev-kill
...
# Kill processes on CLASPER_PORT (8081) and ops live-reload (8083)
dev-kill:
	@-lsof -ti :$${CLASPER_PORT:-8081} | xargs kill 2>/dev/null || true
	@-lsof -ti :8083 | xargs kill 2>/dev/null || true
	@echo "Killed processes on $${CLASPER_PORT:-8081} and 8083 (if any)."
...
# Copy updated Clasper plugin into OpenClaw, then remind to restart gateway.
# Use after editing integrations/openclaw so OpenClaw loads the new code.
# Usage: make openclaw-plugin-reload
openclaw-plugin-reload:
	npx clasper-core setup --profile openclaw --upgrade-openclaw-plugin --non-interactive
	@echo ""
	@echo "  Plugin updated. Restart the OpenClaw gateway to load changes:"
	@echo "    Stop the running gateway (Ctrl+C), then: openclaw gateway start"
	@echo ""
