                ...
                <div style={{ flex: 1 }}>
                  <div style={{ display: "flex", alignItems: "center", gap: "12px", marginBottom: "4px" }}>
                    ...
                    {a.certification_tier && (
                      <span
                        class={`badge-pill ${CERT_TIER_KIND[a.certification_tier] || "muted"}`}
                        data-tooltip={
                          a.certification_tier === "verified"
                            ? "Clasper-tested and verified"
                            : a.certification_tier === "community"
                              ? "Known community adapter"
                              : "Experimental / unverified"
                        }
                      >
                        {CERT_TIER_LABEL[a.certification_tier] ?? titleCase(a.certification_tier)}
                      </span>
                    )}
                  </div>
          ...
          {selectedAdapter && (
            <div style={{ display: "flex", flexDirection: "column", gap: "12px" }}>
              <div class="detail-block" style={{ padding: "12px 16px", marginBottom: 0 }}>
                ...
                {selectedAdapter.certification_tier && (
                  <div class="detail-row">
                    <span class="detail-label" data-tooltip="Adapter certification tier for trust signaling">Certification</span>
                    <span class={`badge-pill ${CERT_TIER_KIND[selectedAdapter.certification_tier] || "muted"}`}>
                      {CERT_TIER_LABEL[selectedAdapter.certification_tier] ?? titleCase(selectedAdapter.certification_tier)}
                    </span>
                  </div>
                )}
                <div class="detail-row">
              ...
              {selectedAdapter.tool_capabilities && selectedAdapter.tool_capabilities.length > 0 && (
                <>
                  <div class="drawer-section-header" style={{ marginTop: "4px", marginBottom: "6px" }}>Tool-level capabilities</div>
                  <div class="detail-block" style={{ padding: "10px 16px", marginBottom: 0 }}>
                    <ul style={{ margin: 0, paddingLeft: "20px", color: "var(--text-primary)", fontSize: "13px", lineHeight: 1.6 }}>
                      {selectedAdapter.tool_capabilities.map((cap, i) => (
                        <li key={i} class="mono">{cap}</li>
                      ))}
                    </ul>
                  </div>
