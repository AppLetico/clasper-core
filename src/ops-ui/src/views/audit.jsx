...
import { SearchIcon, FileIcon, RefreshIcon, XIcon } from "../components/icons.jsx";
...
export function AuditView() {
  ...
  const getEntryAgentRole = (e) =>
    e?.agent_role ?? e?.user_id ?? getEventData(e)?.agent_role ?? getEventData(e)?.actor ?? getEventData(e)?.user_id ?? null;
        ...
        <div class="table-container">
          ...
          {entries && !entries.length && (
            <div class="empty-state">
              <FileIcon width={48} height={48} style={{ opacity: 0.4, marginBottom: 8 }} />
              <div>No audit events found.</div>
              <div class="text-secondary text-xs" style={{ maxWidth: 320 }}>
                Events appear when adapters send tool decisions, approvals, or traces. Try broadening your filters or running a governed tool (e.g. via OpenClaw).
              </div>
            </div>
          )}
          {entries && entries.length > 0 && (
            <table class="data-table">
              <thead><tr>
                <th data-tooltip={copy.tooltips.audit.time}>Time</th>
                <th data-tooltip={copy.tooltips.audit.eventType}>Event Type</th>
                <th data-tooltip={copy.tooltips.audit.whatHappened}>What Happened</th>
                <th data-tooltip={copy.tooltips.audit.agentRole}>Agent Role</th>
                <th data-tooltip={copy.tooltips.audit.request}>Request</th>
              </tr></thead>
              <tbody>
                {entries.map((e, i) => (
                  <tr key={i} onClick={() => openEntry(e)} style={{ cursor: "pointer" }}>
                    <td class="mono text-secondary" title={getEntryTimestamp(e)}>
                      {formatTimestamp(getEntryTimestamp(e))}
                    </td>
...
function AuditEntryDrawer({ entry, onClose }) {
  ...
  const formatTime = (value) => formatTimestamp(value);
  const json = (value) => {
    try {
      ...
      <div class={`drawer ${entry ? "open" : ""}`}>
        ...
        <div class="drawer-body">
          <div class="detail-block">
            ...
            {(entry.agent_role || entry.actor || entry.user_id) && (
              <div class="detail-row">
                <span class="detail-label">{entry.agent_role ? "Agent Role" : "Actor"}</span>
                <span class="mono">{entry.agent_role || entry.actor || entry.user_id}</span>
              </div>
            )}
            <div class="detail-row">
