...
describe("Policy wizard guardrails", () => {
  ...
  it("reconcile endpoint auto-resolves matching pending approvals", async () => {
    ...
    const tenantId = "local";
    const workspaceId = "local";
    const testWorkspaceRoot = path.join(os.tmpdir(), "clasper-reconcile-test");
    const pendingDecision = createDecision({
      ...
      requestSnapshot: {
        request: {
          ...
          context: {
            targets: [testWorkspaceRoot],
            exec: { argv0: "ls", cwd: testWorkspaceRoot },
          },
          templateVars: {
            "workspace.root": testWorkspaceRoot,
          },
        },
      },
    });
