...
function ensureOpenClawConfig(
  ...
  // Note: We intentionally do not set plugins.allow here. Adding it before/during
  // install causes "plugin not found" errors because the plugin isn't on disk yet.
  // Users can add plugins.allow: ["clasper-openclaw"] manually to silence the
  // "plugins.allow is empty" warning after the plugin is installed.
...
function clearOpenClawPluginEntries(): { path: string; changed: boolean } {
  ...
  const hadAllow = allow.includes("clasper-openclaw");

  if (!hadLegacy && !hadClasper && !hadAllow) {
    return { path: configPath, changed: false };
  }
  ...
  plugins.entries = entries;
  if (hadAllow) {
    const filtered = (allow as string[]).filter((id) => id !== "clasper-openclaw");
...
async function syncOpenClawPlugin(
  ...
  // Write config after install so OpenClaw doesn't see a stale plugins.entries.clasper-openclaw (plugin not found).
