...
export interface PolicyContext {
  ...
  tool_group?: string;
  /** Generic action (e.g. exec, model.generate). When absent, tool is used. */
  action?: string;
  /** Generic resource (e.g. command, external_url). */
  resource?: string;
  /** Actor identifier (e.g. agent_id). Enables generic policy matching. */
  actor?: string;
  adapter_id?: string;
...
export interface PolicyEvaluationResult {
  ...
  explanation?: string;
  /** SHA-256 hash of the policy bundle at evaluation time (for deterministic replay). */
...
function subjectMatches(policy: PolicyObject, ctx: PolicyContext): boolean {
  ...
  if (subject.type === 'agent') {
    return !subject.name || subject.name === ctx.agent_id || subject.name === ctx.agent_role;
  }
...
function conditionsMatch(policy: PolicyObject, ctx: PolicyContext): boolean {
  ...
  const effectiveAction = ctx.action ?? ctx.tool;
  if (conditions.tool && (typeof conditions.tool !== 'string' || conditions.tool !== ctx.tool))
    return false;
  if (conditions.action && (typeof conditions.action !== 'string' || conditions.action !== effectiveAction))
    return false;
  if (conditions.resource && (typeof conditions.resource !== 'string' || conditions.resource !== ctx.resource))
    return false;
  const effectiveActor = ctx.actor ?? ctx.agent_id;
  if (conditions.actor && (typeof conditions.actor !== 'string' || conditions.actor !== effectiveActor))
    return false;
  if (
  ...
  if (
    conditions.agent_id &&
    (typeof conditions.agent_id !== 'string' || conditions.agent_id !== ctx.agent_id)
  ) {
    return false;
  }
  if (
    conditions.agent_role &&
    (typeof conditions.agent_role !== 'string' || conditions.agent_role !== ctx.agent_role)
  ) {
    return false;
  }
...
function conditionsMatchExtended(
  ...
  const fieldMap: Array<{ field: string; condition: unknown; actual: unknown }> = [
    ...
    { field: 'action', condition: conditions.action, actual: ctx.action ?? ctx.tool },
    { field: 'resource', condition: conditions.resource, actual: ctx.resource },
    { field: 'actor', condition: conditions.actor, actual: ctx.actor ?? ctx.agent_id },
    ...
    { field: 'agent_id', condition: conditions.agent_id, actual: ctx.agent_id },
    { field: 'agent_role', condition: conditions.agent_role, actual: ctx.agent_role },
