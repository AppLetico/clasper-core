...
export type GovernanceView = {
  ...
  /** Risk score 0–100 from governance evaluation. */
  risk_score: number | null;
  /** Risk level: low, medium, high, critical. */
  risk_level: string | null;
  /** SHA-256 hash of the policy bundle at evaluation time. */
  policy_bundle_hash: string | null;
  /** Decision trace (policy evaluation steps) from the decision record. */
  decision_trace: Array<{ policy_id: string; result: string; decision?: string; explanation?: string }> | null;
};
...
function extractRiskScoreFromDecisionSnapshot(snapshotJson: string | null): number | null {
  if (!snapshotJson) return null;
  try {
    const snap = JSON.parse(snapshotJson) as any;
    const v = snap?.decision?.risk_score;
...
function extractRiskLevelFromDecisionSnapshot(snapshotJson: string | null): string | null {
  if (!snapshotJson) return null;
  try {
    const snap = JSON.parse(snapshotJson) as any;
    const v = snap?.decision?.risk_level;
...
function extractPolicyBundleHashFromDecisionSnapshot(snapshotJson: string | null): string | null {
  if (!snapshotJson) return null;
  try {
    const snap = JSON.parse(snapshotJson) as any;
    const v = snap?.decision?.policy_bundle_hash;
    return typeof v === "string" ? v : null;
  } catch {
    return null;
  }
...
function extractDecisionTraceFromDecisionSnapshot(snapshotJson: string | null): GovernanceView["decision_trace"] {
  if (!snapshotJson) return null;
  try {
    const snap = JSON.parse(snapshotJson) as any;
    const trace = snap?.decision?.decision_trace;
...
export function buildGovernanceView(params: {
  ...
  } else if (params.decisionRow?.status === "deny" || params.decisionRow?.status === "denied" || params.decisionRow?.status === "rejected") {
    decision = "deny";
  } else if (params.decisionRow?.status === "allow") {
    decision = "allow";
  ...
  const snapshotJson = params.decisionRow?.request_snapshot || null;
  return {
    ...
    policy_fallback_hit: extractPolicyFallbackHitFromDecisionSnapshot(snapshotJson),
    denied_tools,
    scope_delta,
    decision_summary,
    risk_score: extractRiskScoreFromDecisionSnapshot(snapshotJson),
    risk_level: extractRiskLevelFromDecisionSnapshot(snapshotJson),
    policy_bundle_hash: extractPolicyBundleHashFromDecisionSnapshot(snapshotJson),
    decision_trace: extractDecisionTraceFromDecisionSnapshot(snapshotJson),
  };
}
