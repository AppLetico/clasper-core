...
export interface TraceSummaryView {
  ...
  workspace_id: string;
  agent_role?: string;
  /** Agent identifier (e.g. from labels.agent_id). Enables per-agent policy matching. */
  agent_id?: string;
  adapter_id?: string;
  environment: string;
  started_at: string;
  ...
  tool_count: number;
  tool_names: string[];
  /** Truncated preview of what the agent did (e.g. command for exec, path for read) */
  action_preview?: string;
  /** Error message or denial reason for incidents (truncated for list view) */
  error?: string;
...
export interface TraceDetailView extends TraceSummaryView {
  /** Error message when execution failed; used to explain "Why it failed" in the UI */
  error?: string;
  input: {
    message: string;
...
function deriveEnvironment(trace: AgentTrace): string {
  ...
  const env = labels.environment || labels.env;
  if (env) return env;
  // Fallback for OpenClaw and similar adapters when no env label was set (e.g. blocked traces)
  if (trace.adapter_id?.startsWith("openclaw")) return "local";
  return "unknown";
}
...
const ACTION_PREVIEW_MAX = 60;

function extractActionPreview(steps: TraceStep[] | undefined): string | undefined {
  const toolCall = steps?.find((s) => s.type === "tool_call");
  if (!toolCall?.data) return undefined;
  const d = toolCall.data as { toolName?: string; arguments?: Record<string, unknown> };
  const args = d.arguments || {};
  const tool = d.toolName || "";
  if (tool === "exec" || tool === "bash" || tool === "process") {
    const cmd = args.command ?? args.cmd ?? (Array.isArray(args.argv) ? (args.argv as string[]).join(" ") : null);
    if (!cmd) return undefined;
    const s = String(cmd);
    return s.length > ACTION_PREVIEW_MAX ? s.slice(0, ACTION_PREVIEW_MAX) + "…" : s;
  }
...
export function buildTraceSummaryView(params: {
  ...
  const labels = trace.labels || {};
  const toolNames = getToolNames(trace.steps);
  const actionPreview = extractActionPreview(trace.steps);
  const requestedCapabilities =
    trace.granted_scope?.capabilities?.length
      ? trace.granted_scope.capabilities
      : getToolNames(trace.steps);
  const annotationsMap = buildAnnotationsMap(annotations);
  const environment = deriveEnvironment(trace);
  const errStep = trace.steps?.find((s) => s.type === "error");
  const errMsg = (errStep?.data as { message?: string } | undefined)?.message;
  const executionError = trace.error || errMsg;
  const denialReason =
    governance?.decision === "deny" && governance.decision_summary ? governance.decision_summary : undefined;
  const errorDetail = typeof executionError === "string" ? executionError : denialReason;
  const errorPreview =
    errorDetail && typeof errorDetail === "string"
      ? errorDetail.length > 100
        ? errorDetail.slice(0, 97) + "…"
        : errorDetail
      : undefined;
  ...
  return {
    ...
    agent_id: (labels as Record<string, string | undefined>)?.agent_id,
...
export function buildTraceDetailView(params: {
  ...
  const errStep = trace.steps.find((s) => s.type === "error");
  const errMsg =
    (errStep?.data as { message?: string } | undefined)?.message;
  const errorFromSteps = trace.error || errMsg;

  return {
    ...summary,
    error: typeof errorFromSteps === "string" ? errorFromSteps : undefined,
    input: {
