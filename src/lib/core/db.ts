...
export function initDatabase(): void {
  ...
  // Migration: add adapter certification and tool-level capabilities
  try {
    db.exec(`ALTER TABLE adapter_registry ADD COLUMN certification_tier TEXT DEFAULT 'experimental'`);
  } catch {
    // Column already exists — ignore
  }
  try {
    db.exec(`ALTER TABLE adapter_registry ADD COLUMN tool_capabilities JSON`);
  } catch {
    // Column already exists — ignore
  }
