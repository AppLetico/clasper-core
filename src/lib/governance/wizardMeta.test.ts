...
describe("wizard meta helpers", () => {
  ...
  it("hashPolicyBundle produces stable hash for same policy set", () => {
    const p1 = { policy_id: "a", effect: { decision: "allow" }, precedence: 10 };
    const p2 = { policy_id: "b", effect: { decision: "deny" }, precedence: 20 };
    const h1 = hashPolicyBundle([p1, p2]);
    const h2 = hashPolicyBundle([p2, p1]);
    expect(h1).toBe(h2);
