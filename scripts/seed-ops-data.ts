...
async function registerAdapter(params: {
  ...
  version: string;
  enabled: boolean;
  certification_tier?: "verified" | "community" | "experimental";
...
async function run() {
  ...
  const adapterA = {
    ...
    tool_capabilities: ["read.file", "write.file", "web_fetch.fetch"],
  };
  const adapterB = {
    ...
    version: "0.1.0",
    enabled: true,
    certification_tier: "experimental" as const,
    tool_capabilities: ["exec.run", "exec.bash", "database.query", "email.send"],
  };
