...
/** All OpenClaw built-in tools (per https://docs.openclaw.ai/tools). Enables policy decisions for every tool. */
...
function inferToolGroup(toolName: string): string | undefined {
  ...
  if (['message', 'whatsapp_login', 'slack', 'discord'].includes(toolName)) {
    return 'messaging';
  }
  if (['sessions_list', 'sessions_history', 'sessions_send', 'sessions_spawn', 'subagents', 'session_status'].includes(toolName)) {
    return 'sessions';
  }
  if (toolName === 'memory_search' || toolName === 'memory_get') {
    return 'memory';
  }
  if (toolName === 'browser' || toolName === 'canvas') {
    return 'ui';
  }
  if (toolName === 'cron' || toolName === 'gateway') {
    return 'automation';
  }
  if (toolName === 'nodes') {
    return 'nodes';
  }
  if (toolName === 'image' || toolName === 'pdf' || toolName === 'tts') {
    return 'media';
  }
  return undefined;
}
...
export async function register(api: OpenClawPluginApi): Promise<void> {
  ...
  /** Resolve environment for trace labels. Tries: plugin config → OpenClaw config → env vars → 'local'. */
  const resolveEnvironment = (): string => {
    const s = (v: unknown) => (typeof v === 'string' && v.length > 0 ? v : null);
    return (
      s(config.environment) ??
      s(api.config && typeof api.config === 'object' ? (api.config as Record<string, unknown>).environment : null) ??
      s(api.config && typeof api.config === 'object' ? (api.config as Record<string, unknown>).env : null) ??
      s(typeof process !== 'undefined' && process.env?.OPENCLAW_ENV) ??
      s(typeof process !== 'undefined' && process.env?.CLASPER_ENV) ??
      s(typeof process !== 'undefined' && process.env?.NODE_ENV) ??
      'local'
    );
  };
  ...
  api.on!(
    ...
    async (
      ...
      try {
        ...
        const toolContext = mapToolContext(
          { name: toolName, group: toolGroup },
          params,
          { event, sessionContext: context }
        );
        ...
        if (effect === 'allow') {
          ...
          // Push to pending so after_tool_call can ingest the trace
  ...
  api.on!(
    ...
    async (event: Record<string, unknown>, context: Record<string, unknown>): Promise<void> => {
      ...
      const key = invocationKey(context, rawToolName);
      let queue = pendingByKey.get(key);
      let matchedKey = key;

      // Fallback: if primary key misses (context may differ between hooks), find any pending for this tool
      if (!queue || queue.length === 0) {
        const toolLower = rawToolName.toLowerCase();
        for (const [k, q] of pendingByKey) {
          if (q.length > 0 && q[0].rawToolName.toLowerCase() === toolLower) {
            queue = q;
            matchedKey = k;
            break;
          }
      ...
      if (queue.length === 0) pendingByKey.delete(matchedKey);
