...
openclaw agent --session-id "clasper-test-$(date +%s)" -m "Use the read tool to read $HOME/.openclaw/workspace/README.md and return only the first line."
```
...
```bash
openclaw agent -m "Call the read tool with path $HOME/.openclaw/workspace/README.md. Reply with only the first line of the file."
```
...
## Quick Reference: Tools and Policies

| Effect | Tools |
|--------|-------|
| **allow** | read, sessions_list, sessions_history, session_status, memory_search, memory_get, process, agents_list, image, pdf, canvas, tts |
| **require_approval** | exec, write, web_search, web_fetch, sessions_send, sessions_spawn, message, whatsapp_login, slack, discord, subagents, edit, apply_patch, nodes, browser, cron, gateway |
| **deny** | delete |

**Approval flow:** When the chat shows "...", run `openclaw logs --follow` or open the Control UI Logs tab; the approval URL is logged. Go to Ops Console → Approvals to approve or deny.

**Filtering traces:** In the Traces view, use the Adapter dropdown to filter by `openclaw-local` (or other adapter IDs).

**Adding new tools:** Edit `policies/openclaw-default.yaml`, add a policy, then re-run `npx clasper-core seed openclaw`. See the full policy file for structure and precedence rules.
...
├── policies/
│   └── openclaw-default.yaml     # Default governance policies
├── demos/
