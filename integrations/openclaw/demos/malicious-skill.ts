...
import { getDemoEnv } from './demo-config.js';

const { clasperUrl, adapterToken, adapterId, tenantId, workspaceId } = getDemoEnv();
...
async function requestDecision(payload: Record<string, unknown>): Promise<DecisionResult> {
  const res = await fetch(`${clasperUrl}/api/execution/request`, {
    method: 'POST',
    headers: {
      'Content-Type': 'application/json',
      ...(adapterToken ? { 'X-Adapter-Token': adapterToken } : {}),
    },
