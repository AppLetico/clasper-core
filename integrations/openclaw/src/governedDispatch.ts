...
export interface GovernedDispatchConfig {
  ...
  /** Clasper base URL for building approvals link (e.g. http://localhost:8082). */
  clasperUrl?: string;
}
...
export function createGovernedDispatch(
  ...
  const approvalsUrl = config.clasperUrl ? `${config.clasperUrl.replace(/\/+$/, '')}/ops#approvals` : undefined;
  ...
  return async function governedToolInvoke(
    ...
    switch (effect) {
      ...
      case 'pending': {
        ...
        log(
          ...
          (approvalsUrl ? `. Approve in Clasper Ops Console: ${approvalsUrl}` : '')
        );
        try {
    ...
    // Ingest trace so it appears in the Clasper Ops Console
    await client.ingestTrace({
      ...
      trace: {
        ...
        output: {
          message: `Tool ${normalizedTool.name} executed`,
          toolCalls: [
            {
              id: executionId,
